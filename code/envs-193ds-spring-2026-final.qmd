...
nests_mod_preds <- ggpredict(nests_mod2,
                             terms = "height_cm [all]")
...
ggplot(data = nests_clean,
  ...
  geom_point(size = 3,
             alpha = 0.4) +
  geom_line(data = nests_mod_preds,
            aes(x = x,
                y = predicted)) +
  geom_ribbon(data = nests_mod_preds,
              aes(x = x,
                  y = predicted,
                  ymin = conf.low,
                  ymax = conf.high),
...
**Figure 1. Daily Duration of Activities Effect on Time Spent Biking** As duration of day's activities increases, daily total time spent biking also increases. Individual daily observations are shown as blue dots on the graph as well as a 95% confidence interval.
